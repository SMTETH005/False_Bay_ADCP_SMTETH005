...
def review_plot_naming(plot_title, png_name, serial_number,
                       is_pre_split=False, resampled=None, time_tag=None):
    """
    Final plot title and png name amendments, including optional time tags.
    """

    # Handle unknown serial number
    if serial_number == 'Unknown':
        if plot_title is not None:
            plot_title = plot_title.replace('Unknown ', '')
        png_name = png_name.replace('Unknown_', '')
    ...
    if time_tag:
        if plot_title is not None:
            plot_title += f' {time_tag}'
        png_name = png_name.replace('.png', f'{time_tag}.png')
...
def make_plot_tidal_ellipses(dest_dir: str, data_filename, station: str, deployment_number: str,
    ...
    #ell_scale = optim.x[0] # had to remove this to get tidal plots for GB4 and KB7 to work.
    #ell_scale = optim.x # replaced the line abvove with this and it works now
    # automatically choose the correct ell_scale
    # one-liner for ell_scale
    ell_scale = np.atleast_1d(optim.x)[0]
...
def create_westcoast_plots(
        ...
        do_filter_ac=False,time_slice=None,
):
    """
    Inputs:
    ...
    if time_slice is not None:
        ...
        ncdata = ncdata.sel(time=slice(start, end))
        try:
            start_str = np.datetime_as_string(np.array(ncdata.time.values[0], dtype="datetime64[s]"), unit="s")
            end_str = np.datetime_as_string(np.array(ncdata.time.values[-1], dtype="datetime64[s]"), unit="s")
        except Exception:
            start_str, end_str = str(start), str(end)
        time_tag = f"_{start_str[:10].replace('-', '')}-{end_str[:10].replace('-', '')}"
    else:
        time_tag = ""
