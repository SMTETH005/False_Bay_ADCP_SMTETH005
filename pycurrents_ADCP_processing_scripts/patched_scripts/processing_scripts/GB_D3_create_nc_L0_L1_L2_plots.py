...
f = './project_data/updated_project_data/SMTETH005_GB_RDIWH_20151203_20160304_D3/SMTETH005_GB_RDIWH_20151203_20160304_D3.pd0' 
meta = './project_data/updated_project_data/SMTETH005_GB_RDIWH_20151203_20160304_D3/SMTETH005_GB_RDIWH_20151203_20160304_D3_metadata.csv'
dest_dir = './processed_data/SMTETH005_pycurrents_processed_GB_RDIWH_20151203_20160304_D3'
...
for n in ncnames_L1:
    ...
    plot_list = plot_westcoast_nc_LX.create_westcoast_plots(
        ...
        colourmap_lim=(-0.8, 0.8),     # Limits for all pcolormesh plots in m/s
        time_slice=None, #("2016-01-01 00:00:00", "2016-02-01 00:00:00"), # set to None for full time range otherwise ("2015-06-01 00:00:00", "2015-07-01 00:00:00")
        override_resample=False,       # Set True to skip downsampling for large files
        do_all_plots=False,            # Set False to customise which plots are generated

        # --- Basic QC & diagnostics ---
        do_diagnostic=False,            # Plots for quality control
        do_pressure=True,              # Pressure (PRESPR01) vs time

        # --- Velocity pcolormesh plots ---
        do_ne=False,                    # North/East velocity pcolormesh
        do_ac=False,                    # Alongshore/Cross-shore velocity pcolormesh
        do_filter_ne=False,             # Filtered NE pcolormesh
        do_filter_ac=False,             # Filtered AC pcolormesh

        # --- Single-bin plots ---
        do_single_bin_ne=True,         # Single-bin NE velocity time series
        #single_bin_depths=[3,5,10,15,20], # Depths (m) for single-bin plots # can be None for automatic selection or ex: [10, 30, 50]
        # single_bin_inds can be used alternatively to select specific bin indices

        # --- Feather / quiver plots ---
        do_quiver=False,                # Vector plots (velocity arrows)

        # --- Spectral plots ---
        do_single_rotary_spectra=True, # Rotary spectra for single bins
        do_profile_rotary_spectra=False,# Depth-profile of rotary spectra
        do_tidal=False                  # Tidal ellipses / tidal analysis plots
    )
...
for f_adcp in ncnames_L1:
    ...
    for n in out_files:
        plot_list_L2 = plot_westcoast_nc_LX.create_westcoast_plots(
            ncfile=n,
            dest_dir=dest_dir,
            filter_type="Godin",
            along_angle=137, 
            colourmap_lim=(-0.8, 0.8),
            time_slice=("2016-01-01 00:00:00", "2016-02-01 00:00:00"),
            override_resample=False,
            do_all_plots=False,   # or True if you want the full set
            ...
            #single_bin_depths=[3,5,10,15,20],
