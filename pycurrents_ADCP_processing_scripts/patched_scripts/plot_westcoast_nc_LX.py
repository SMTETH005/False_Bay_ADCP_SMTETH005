...
plt.rcParams.update({
    ...
    'figure.titlesize': 16,    # Figure title (suptitle)
})
...
def make_pcolor_ne(nc: xr.Dataset, dest_dir, time_lim, bin_depths_lim,
    ...
    ax.invert_yaxis()
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=30, ha='right')  # Rotate labels
    ...
    ax2.invert_yaxis()
    plt.setp(ax2.xaxis.get_majorticklabels(), rotation=30, ha='right')  # Rotate labels
...
def make_pcolor_ac(nc: xr.Dataset, dest_dir, time_lim, bin_depths_lim, ns_lim, ew_lim,
    ...
    ax1.invert_yaxis()
    plt.setp(ax1.xaxis.get_majorticklabels(), rotation=30, ha='right')  # Rotate labels
    ...
    plt.tight_layout()  # Adjust layout to prevent label cutoff
    fig.savefig(plot_dir + plot_name, dpi = 800)
    plt.close()
...
def plot_rot(r: dict, clabel=None, cx=None, cy=None, color=None, ccolor='k', units='m/s', funits='cpd',
    ...
    if fig is None and (ax_pos is None and ax_neg is None):
        fig = plt.figure(figsize=(10.5, 5.5))  # (width, height) so the subplots are more square 8.5, 4.5 
...
def make_plot_tidal_ellipses(dest_dir: str, data_filename, station: str, deployment_number: str,
    ...
    for ax in axes:
        ...
        ax.tick_params(axis='x', labelsize=12)  # Set x-axis tick label size to 12
